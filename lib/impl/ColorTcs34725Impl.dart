...
class ColorAdcData extends DataTypeBase {
  ColorAdcData.Default()
      : super(
            ModuleType.COLOR_DETECTOR,
            Util.setSilentRead(ColorTcs34725Impl.ADC),
  ...
  Data createMessage(bool logData, MetaWearBoardPrivate mwPrivate,
    ...
    return DataPrivate2(timestamp, data, apply, () => 1.0, <T>() {
      if (T is ColorAdc) return wrapper as T;
    });
  }
      ...
      case Combiner.ID:
        {
          ...
          return Tuple2(
              new UintData(ModuleType.DATA_PROCESSOR, DataProcessorImpl.NOTIFY,
                  attributes,
                  input: this),
              null);
        }
    ...
    switch (Util.clearRead(dataType.eventConfig[1])) {
      case ADC:
        return dataType.attributes.length() > 2
            ? "color"
            : sprintf("color[%d]", [dataType.attributes.offset >> 1]);
      default:
        return null;
    }
  }

  static const String ADC_PRODUCER =
          "com.mbientlab.metawear.impl.ColorTcs34725Impl.ADC_PRODUCER",
      ADC_CLEAR_PRODUCER =
          "com.mbientlab.metawear.impl.ColorTcs34725Impl.ADC_CLEAR_PRODUCER",
      ADC_RED_PRODUCER =
          "com.mbientlab.metawear.impl.ColorTcs34725Impl.ADC_RED_PRODUCER",
      ADC_GREEN_PRODUCER =
          "com.mbientlab.metawear.impl.ColorTcs34725Impl.ADC_GREEN_PRODUCER",
      ADC_BLUE_PRODUCER =
          "com.mbientlab.metawear.impl.ColorTcs34725Impl.ADC_BLUE_PRODUCER";
