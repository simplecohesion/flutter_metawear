...
class EventImpl extends ModuleImplBase implements Module {
  ...
  void init() {
    mwPrivate.addResponseHandler(Tuple2(ModuleType.EVENT.id, ENTRY),
  ...
  void tearDown() {
    mwPrivate.sendCommand(
        Uint8List.fromList([ModuleType.EVENT.id, EventImpl.REMOVE_ALL]));
  }

  void removeEventCommand(int id) {
    mwPrivate.sendCommand(
        Uint8List.fromList([ModuleType.EVENT.id, EventImpl.REMOVE, id]));
  }

  Future<List<int>> queueEvents(
      final Queue<Tuple2<DataTypeBase, CodeBlock>> eventCodeBlocks) async {
    ...
    try {
      while (!eventCodeBlocks.isEmpty) {
        Tuple2<DataTypeBase, CodeBlock> entry = eventCodeBlocks.removeLast();
        ...
        while (!recordedCommands.isEmpty) {
          ...
          exception = TimeoutException(
              "Did not receive event id", ModuleType.RESPONSE_TIMEOUT);
  ...
  void convertToEventCommand(Uint8List command) {
    Uint8List commandEntry = Uint8List.fromList([
      ModuleType.EVENT.id,
      EventImpl.ENTRY,
      activeDataType.eventConfig[0],
      activeDataType.eventConfig[1],
      activeDataType.eventConfig[2],
      command[0],
      command[1],
      (command.length - 2)
    ]);
    ...
    if (feedbackParams != null) {
      ...
      tempEntry[commandEntry.length] = (0x01 |
          ((feedbackParams.item1 << 1) & 0xff) |
          ((feedbackParams.item2 << 4) & 0xff));
