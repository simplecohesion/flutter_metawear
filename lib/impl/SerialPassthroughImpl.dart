...
class SerialPassthroughData extends ByteArrayData {
  SerialPassthroughData.Default(int register, int id, int length)
      : super(ModuleType.SERIAL_PASSTHROUGH, register,
            new DataAttributes(Uint8List.fromList([length]), 1, 0, false),
            id: id);

  SerialPassthroughData(DataTypeBase input, ModuleType module, int register,
...
class _SpiParameterBuilderInner extends SpiParameterBuilderInner<void> {
  ...
  _SpiParameterBuilderInner.value(
      int fifthValue, this._spiProducer, this._mwPrivate)
...
class I2cInner implements I2C {
  ...
  I2cInner(this.id, int length, this.mwPrivate) {
    mwPrivate.tagProducer(
        name(),
        new SerialPassthroughData.Default(
            Util.setSilentRead(SerialPassthroughImpl.I2C_RW), id, length));
  }
  ...
  void read(int deviceAddr, int registerAddr) {
    ...
    i2cProducer.read(
        mwPrivate,
        Uint8List.fromList(
            [deviceAddr, registerAddr, id, i2cProducer.attributes.length()]));
  }
...
class SpiInner implements SPI {
  ...
  SpiInner(this.id, int length, this.mwPrivate) {
    mwPrivate.tagProducer(
        name(),
        new SerialPassthroughData.Default(
            Util.setSilentRead(SerialPassthroughImpl.SPI_RW), id, length));
  }
  ...
  SpiParameterBuilder<void> read() {
    ...
    return _SpiParameterBuilderInner.value(
        ((spiProducer.attributes.length() - 1) | (id << 4)),
        spiProducer,
        mwPrivate);
  }
...
abstract class SpiParameterBuilderInner<T> implements SpiParameterBuilder<T> {
  ...
  SpiParameterBuilderInner()
      : originalLength = 5,
        this.config = Uint8List(5);

  SpiParameterBuilderInner.value(int fifthValue)
      : this.originalLength = 6,
        this.config = Uint8List(6) {
    config[5] = fifthValue;
  }
...
class _SpiParameterBuilderInner3 extends SpiParameterBuilderInner {
  ...
  commit() {
    _mwPrivate.sendCommandForModule(
        ModuleType.SERIAL_PASSTHROUGH, SerialPassthroughImpl.SPI_RW, config);
    return null;
  }
}

class _SpiParameterBuilderInner1
    extends SpiParameterBuilderInner<Future<Uint8List>> {
  final StreamController<Uint8List> _readControllerSpiController;
  final MetaWearBoardPrivate _mwPrivate;

  _SpiParameterBuilderInner1.value(
      int fifthValue, this._readControllerSpiController, this._mwPrivate)
      : super.value(fifthValue);

  _SpiParameterBuilderInner1(this._readControllerSpiController, this._mwPrivate)
      : super();

  @override
  commit() async {
    Stream<Uint8List> stream = _readControllerSpiController.stream
 ...
 */
class SerialPassthroughImpl extends ModuleImplBase
    implements SerialPassthrough {
  ...
  static const String I2C_PRODUCER_FORMAT =
          "com.mbientlab.metawear.impl.SerialPassthroughImpl.I2C_PRODUCER_%d",
      SPI_PRODUCER_FORMAT =
          "com.mbientlab.metawear.impl.SerialPassthroughImpl.SPI_PRODUCER_%d";

  final Map<int, I2C> i2cDataProducers = Map();
  final Map<int, SPI> spiDataProducers = Map();
  final StreamController<Uint8List> _readControllerI2cController =
  ...
  void init() {
    ...
    mwPrivate.addDataHandler(
        Tuple3(ModuleType.SERIAL_PASSTHROUGH.id, Util.setRead(I2C_RW),
            DIRECT_I2C_READ_ID),
        (Uint8List response) => _readControllerI2cController.add(response));

    mwPrivate.addDataIdHeader(
        Tuple2(ModuleType.SERIAL_PASSTHROUGH.id, Util.setRead(SPI_RW)));
    mwPrivate.addDataHandler(
        Tuple3(ModuleType.SERIAL_PASSTHROUGH.id, Util.setRead(SPI_RW),
            DIRECT_SPI_READ_ID),
        (Uint8List response) => _readControllerSpiController.add(response));
  }
  ...
  @override
  Future<Uint8List> readI2cAsync(
      final int deviceAddr, final int registerAddr, final int length) async {
    Stream<Uint8List> stream = _readControllerI2cController.stream
        .timeout(ModuleType.RESPONSE_TIMEOUT);
    StreamIterator<Uint8List> iterator = StreamIterator(stream);
  ...
  SPI spi(final int length, final int id) {
    if (mwPrivate.lookupModuleInfo(ModuleType.SERIAL_PASSTHROUGH).revision <
        SPI_REVISION) {
      return null;
    }
  ...
  SpiParameterBuilder<void> writeSpi() {
    return mwPrivate.lookupModuleInfo(ModuleType.SERIAL_PASSTHROUGH).revision >=
            SPI_REVISION
