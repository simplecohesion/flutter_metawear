...
import 'package:flutter_metawear/data/euler_angles.dart';
...
class EulerAngleData extends DataTypeBase {
  EulerAngleData.Default()
      : super(ModuleType.SENSOR_FUSION, SensorFusionBoschImpl.EULER_ANGLES,
  ...
  Data createMessage(bool logData, MetaWearBoardPrivate mwPrivate,
    ...
    return DataPrivate2(timestamp, data, apply, () => 1.0, <T>() {
      if (T is EulerAngles) {
        return EulerAngles(values[0], values[1], values[2], values[3]) as T;
...
class QuaternionData extends DataTypeBase {
  QuaternionData.Default()
      : super(ModuleType.SENSOR_FUSION, SensorFusionBoschImpl.QUATERNION,
            new DataAttributes(Uint8List.fromList([4, 4, 4, 4]), 1, 0, true));

  QuaternionData(DataTypeBase input, ModuleType module, int register, int id,
      DataAttributes attributes)
  ...
  Data createMessage(bool logData, MetaWearBoardPrivate mwPrivate,
    ...
    return DataPrivate2(timestamp, data, apply, () => 1.0, <T>() {
      if (T is EulerAngles) {
        return Quaternion(values[0], values[1], values[2], values[3]) as T;
      } else if (T is List<double>) {
        return values as T;
      }
      throw CastError();
...
class AccelerationData extends DataTypeBase {
  ...
  AccelerationData.Default(int register)
      : super(ModuleType.SENSOR_FUSION, register,
            new DataAttributes(Uint8List.fromList([4, 4, 4]), 1, 0, true));

  AccelerationData(DataTypeBase input, ModuleType module, int register, int id,
      DataAttributes attributes)
      : super(module, register, attributes, id: id, input: input);
...
abstract class CorrectedSensorData extends DataTypeBase {
  CorrectedSensorData.Default(int register)
      : super(ModuleType.SENSOR_FUSION, register,
...
class CorrectedAccelerationData extends CorrectedSensorData {
  ...
  CorrectedAccelerationData(DataTypeBase input, ModuleType module, int register,
      int id, DataAttributes attributes)
...
class _ConfigEditor extends ConfigEditor {
  ...
  List<dynamic> extraAcc = null, extraGyro = null;
  ...
  void commit() {
    ...
    Accelerometer.Accelerometer acc =
        _impl.mwPrivate.getModules()[Accelerometer.Accelerometer]
            as Accelerometer.Accelerometer;
    GyroBmi160.GyroBmi160 gyro = _impl.mwPrivate
        .getModules()[GyroBmi160.GyroBmi160] as GyroBmi160.GyroBmi160;
    MagnetometerBmm150.MagnetometerBmm150 mag =
        _impl.mwPrivate.getModules()[MagnetometerBmm150.MagnetometerBmm150]
            as MagnetometerBmm150.MagnetometerBmm150;
    ...
    switch (newMode) {
      ...
      case Mode.NDOF:
        {
          Accelerometer.ConfigEditor accEditor = acc.configure().odr(100).range(
              AccelerometerBosch.AccRange.values[newAccRange.index].range);
          if (acc is AccelerometerBmi160.AccelerometerBmi160) {
            addExtraAcc(accEditor);
          }
          accEditor.commit();

          GyroBmi160.ConfigEditor gyroEditor = gyro
              .configure()
      ...
      case Mode.COMPASS:
        {
          Accelerometer.ConfigEditor accEditor = acc.configure().odr(25).range(
              AccelerometerBosch.AccRange.values[newAccRange.index].range);
          if (acc is AccelerometerBmi160.AccelerometerBmi160) {
            addExtraAcc(accEditor);
          }
      ...
      case Mode.M4G:
        {
          Accelerometer.ConfigEditor accEditor = acc.configure().odr(50).range(
              AccelerometerBosch.AccRange.values[newAccRange.index].range);
          if (acc is AccelerometerBmi160.AccelerometerBmi160) {
            addExtraAcc(accEditor);
          }
...
class SensorFusionBoschImpl extends ModuleImplBase
    implements SensorFusionBosch {
  ...
  static const int ENABLE = 1,
      ...
      GYRO_CALIB_DATA = 0xd,
      MAG_CALIB_DATA = 0xe;
  static const String QUATERNION_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.QUATERNION_PRODUCER",
      EULER_ANGLES_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.EULER_ANGLES_PRODUCER",
      GRAVITY_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.GRAVITY_PRODUCER",
      LINEAR_ACC_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.LINEAR_ACC_PRODUCER",
      CORRECTED_ACC_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.CORRECTED_ACC_PRODUCER",
      CORRECTED_ROT_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.CORRECTED_ROT_PRODUCER",
      CORRECTED_MAG_PRODUCER =
          "com.mbientlab.metawear.impl.SensorFusionBoschImpl.CORRECTED_MAG_PRODUCER";

  Mode mode;
  int dataEnableMask;

  final StreamController<Uint8List> _readControllerCalibration =
      StreamController<Uint8List>();

  AsyncDataProducer correctedAccProducer,
      correctedAngVelProducer,
      correctedMagProducer,
      quaterionProducer,
      eulerAnglesProducer,
      gravityProducer,
      linearAccProducer;
  ...
  SensorFusionBoschImpl(MetaWearBoardPrivate mwPrivate) : super(mwPrivate) {
    ...
    mwPrivate.tagProducer(QUATERNION_PRODUCER, new QuaternionData.Default());
    mwPrivate.tagProducer(EULER_ANGLES_PRODUCER, new EulerAngleData.Default());
    mwPrivate.tagProducer(
  ...
  void init() {
    ...
    if (mwPrivate.lookupModuleInfo(ModuleType.SENSOR_FUSION).revision >=
        CALIBRATION_STATE_REV) {
      mwPrivate.addResponseHandler(
          Tuple2(ModuleType.SENSOR_FUSION.id, Util.setRead(CALIB_STATUS)),
          (Uint8List response) => _readControllerCalibration.add(response));
    }
    if (mwPrivate.lookupModuleInfo(ModuleType.SENSOR_FUSION).revision >=
        CALIBRATION_DATA_REV) {
      mwPrivate.addResponseHandler(
          Tuple2(ModuleType.SENSOR_FUSION.id, Util.setRead(ACC_CALIB_DATA)),
          (Uint8List response) => _readControllerCalibration.add(response));
      mwPrivate.addResponseHandler(
          Tuple2(ModuleType.SENSOR_FUSION.id, Util.setRead(GYRO_CALIB_DATA)),
          (Uint8List response) => _readControllerCalibration.add(response));
      mwPrivate.addResponseHandler(
          Tuple2(ModuleType.SENSOR_FUSION.id, Util.setRead(MAG_CALIB_DATA)),
          (Uint8List response) => _readControllerCalibration.add(response));
    }
  }
  ...
  AsyncDataProducer correctedAcceleration() {
    if (correctedAccProducer == null) {
      correctedAccProducer =
          _SensorFusionAsyncDataProducer(this, CORRECTED_ACC_PRODUCER, 0x01);
    }
  ...
  AsyncDataProducer correctedAngularVelocity() {
    if (correctedAngVelProducer == null) {
      correctedAngVelProducer =
          _SensorFusionAsyncDataProducer(this, CORRECTED_ROT_PRODUCER, 0x02);
    }
  ...
  AsyncDataProducer correctedMagneticField() {
    if (correctedMagProducer == null) {
      correctedMagProducer =
          _SensorFusionAsyncDataProducer(this, CORRECTED_MAG_PRODUCER, 0x04);
    }
  ...
  AsyncDataProducer eulerAngles() {
    if (eulerAnglesProducer == null) {
      eulerAnglesProducer =
          _SensorFusionAsyncDataProducer(this, EULER_ANGLES_PRODUCER, 0x10);
    }
  ...
  void start() {
    Accelerometer.Accelerometer acc =
        mwPrivate.getModules()[Accelerometer.Accelerometer]
            as Accelerometer.Accelerometer;
    GyroBmi160.GyroBmi160 gyro =
        mwPrivate.getModules()[GyroBmi160.GyroBmi160] as GyroBmi160.GyroBmi160;
    MagnetometerBmm150.MagnetometerBmm150 mag =
        mwPrivate.getModules()[MagnetometerBmm150.MagnetometerBmm150]
            as MagnetometerBmm150.MagnetometerBmm150;
  ...
  Future<void> pullConfigAsync() async {
    Stream<Uint8List> stream =
        _readControllerCalibration.stream.timeout(ModuleType.RESPONSE_TIMEOUT);
    StreamIterator<Uint8List> iterator = StreamIterator(stream);

    mwPrivate.sendCommand(
        Uint8List.fromList([ModuleType.SENSOR_FUSION.id, Util.setRead(MODE)]));
    TimeoutException exception = TimeoutException(
        "Did not receive sensor fusion config ", ModuleType.RESPONSE_TIMEOUT);
  ...
  Future<CalibrationData> calibrate(
      {void updateHandler(CalibrationState state),
      int pollingPeriod = 1000}) async {
    if (mwPrivate.lookupModuleInfo(ModuleType.SENSOR_FUSION).revision >=
        CALIBRATION_DATA_REV) {
      bool terminate;
      Uint8List acc;
      ...
      while (true) {
        ...
        switch (mode) {
          case Mode.NDOF:
            if (state.accelerometer == CalibrationAccuracy.HIGH_ACCURACY &&
                state.gyroscope == CalibrationAccuracy.HIGH_ACCURACY &&
                state.magnetometer == CalibrationAccuracy.HIGH_ACCURACY)
              terminate = true;
      ...
      if (mode != Mode.IMU_PLUS) {
        exception = TimeoutException("Did not receive magnetometer calibration",
            ModuleType.RESPONSE_TIMEOUT);
        mwPrivate.sendCommand(Uint8List.fromList(
  ...
  void writeCalibrationData(CalibrationData data) {
    if (mwPrivate.lookupModuleInfo(ModuleType.SENSOR_FUSION).revision >=
      ...
      mwPrivate.sendCommandForModule(ModuleType.SENSOR_FUSION,
          SensorFusionBoschImpl.ACC_CALIB_DATA, data.accelerometer);
