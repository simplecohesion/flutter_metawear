...
class ArrayData extends DataTypeBase {
  ...
  Data createMessage(bool logData, MetaWearBoardPrivate mwPrivate,
    ...
    while (!(fuser.editor.configObj is Fuser)) {
      fuser =
          dpModules.activeProcessors[fuser.editor.source.input.eventConfig[2]];
    }
    ...
    int offset = 0;
    final List<Data> unwrappedData = List<Data>(fuser.editor.config.length + 1);
    unwrappedData[0] =
    ...
    for (int i = 2; i < fuser.editor.config.length; i++) {
      Processor value = dpModules.activeProcessors[fuser.editor.config[i]];
