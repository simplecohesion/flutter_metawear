    ...
    if (dataResponseHandler == null) {
      if (source.attributes.copies > 1) {
        ...
        dataResponseHandler = (Uint8List response) {
          ...
          AccountType accountType = accounter == null
              ? AccountType.TIME
          ...
          for (int i = 0,
            ...
            Tuple3<DateTime, int, int> account =
                fillTimestamp(mwPrivate, accounter, response, j);
            Uint8List dataRaw = Uint8List(dataUnitLength - (account.item2 - j));
            dataRaw.setAll(0, response.skip(account.item2));

            call(source.createMessage(
                false,
                mwPrivate,
                dataRaw,
                accounter == null ? now : account.item1,
                accountType == AccountType.TIME
                    ? null
                    : (<T>() => T is int ? account.item3 as T : null)));
          }
        };
      } else {
        dataResponseHandler = (Uint8List response) {
          ...
          if (source.eventConfig[0] == ModuleType.DATA_PROCESSOR.id &&
            ...
            account = fillTimestamp(
                mwPrivate,
                dataprocessor.lookupProcessor(source.eventConfig[2]),
                dataRaw,
                0);
...
//                            System.arraycopy(dataRaw, account.second, copy, 0, copy.length);
          ...
          if (packer != null) {
            ...
            for (int i = 0, j = 3 + account.item2;
                i < packer.editor.source.attributes.copies &&
                    j < response.length;
                i++, j += dataUnitLength) {
...
//                ByteBuffer.wrap(padded).order(ByteOrder.LITTLE_ENDIAN).getLong(0);
        ...
        switch (config.type) {
          ...
          case AccountType.TIME:
            {
              LoggingImpl logging =
                  mwPrivate.getModules()[Logging] as LoggingImpl;
              return Tuple3(
                  logging.computeTimestamp(-1, tick), size + offset, tick);
            }
        }
      }
    }
