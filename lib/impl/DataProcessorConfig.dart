...
class Threshold extends DataProcessorConfig {
  ...
  Threshold(
      this.input, this.isSigned, this.mode, this.boundary, this.hysteresis)
      : super(ID);
...
class Average extends DataProcessorConfig {
  ...
  Uint8List build() {
    ...
    buffer.setUint8(
        offset += 1,
        ((((output - 1) & 0x3) | (((input - 1) & 0x3) << 2)) |
            ((supportsHpf ? (hpf ? 1 : 0) : 0) << 5)));
    buffer.setUint8(offset += 1, samples);
    if (supportsHpf) buffer.setUint8(offset += 1, (nInputs - 1));
    return data;
  }
...
class MultiValueComparison extends ComparisonConfig {
  ...
  MultiValueComparison(
      this.isSigned, this.input, this.op, this.mode, this.references)
  ...
  MultiValueComparison.config(Uint8List config)
      : this.isSigned = (config[1] & 0x1) == 0x1,
        ...
        this.references = extractReferences(
            ByteData.view(config.buffer, 2), (((config[1] >> 1) & 0x3) + 1)),
        super(config[0]);
...
class Maths extends DataProcessorConfig {
  ...
  Uint8List build() {
    ...
    if (multiChnlMath) buffer.setInt8(offset += 1, nInputs - 1);
    return data;
  }
...
class Delay extends DataProcessorConfig {
  ...
  @override
  Uint8List build() =>
      Uint8List.fromList([ID, ((input - 1) & (expanded ? 0xf : 0x3)), samples]);
...
class Packer extends DataProcessorConfig {
  ...
  @override
  String createUri(bool state, int procId) => sprintf("packer?id=%d", [procId]);
}
...
class Fuser extends DataProcessorConfig {
  ...
  Uint8List build() {
    ...
    for (int filterId in filterIds) buffer.setInt8(offset += 1, filterId);
    return data;
  }

  @override
  String createUri(bool state, int procId) => sprintf("fuser?id=%d", [procId]);
}

abstract class DataProcessorConfig {
  static DataProcessorConfig from(
      Version firmware, int revision, Uint8List config) {
    switch (config[0]) {
      ...
      case ComparisonConfig.ID:
        return firmware.compareTo(
                    RouteComponentImpl.MULTI_COMPARISON_MIN_FIRMWARE) >=
                0
            ? new MultiValueComparison.config(config)
            : new SingleValueComparison.config(config);
      case Combiner.ID:
        return Combiner.config(config);
      case Time.ID:
        return Time.config(config);
      case Maths.ID:
        return new Maths.config(
            firmware.compareTo(RouteComponentImpl.MULTI_CHANNEL_MATH) >= 0,
            config);
      case Delay.ID:
        return new Delay.config(
            revision >= DataProcessorImpl.EXPANDED_DELAY, config);
      case Pulse.ID:
